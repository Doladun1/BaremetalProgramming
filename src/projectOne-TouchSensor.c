...
void main()//8bit x
{
    ...
    while(1)
    {
		...
		{
			set_level = (touch_scan()-10000)/128;
			if(color ==blue) blue_count=set_level;
			else if(color == green) green_count=set_level;
			else if(color ==  red) red_count=set_level;
			// color select cycles each axes 	
			set_led = readAccelerometer(color_select);
			
			//if set_led is clear or axis reads positive led is cleared
			if		(color_select ==0 )	red_set = set_led;
			else if (color_select == 2) green_set = set_led;
			else if (color_select == 4) blue_set = set_led;
